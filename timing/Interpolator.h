// Quick Note: this is the Interpolator header provided by the
// displacement interpolation paper with multithreading for ubuntu.
...
#define WINAPI 
#define LPVOID void*
#define DWORD void* // not technically correct, but a quick fix

// --added for ubuntu multithreading-- //
#include <pthread.h>
#include <unistd.h>
#endif
...
{
public:
	Interpolator(const std::vector<Vector<DIM,SAMPLESTYPE> > &pA, const std::vector<double> &pwA, 
				 const std::vector<Vector<DIM,SAMPLESTYPE> > &pB, const std::vector<double> &pwB, 
				 double (*pSqrDist)(const Vector<DIM,SAMPLESTYPE> &, const Vector<DIM,SAMPLESTYPE> &), 
				 double (*pKernel)(const Vector<DIM,SAMPLESTYPE> &, const Vector<DIM,SAMPLESTYPE> &, double),
				 Vector<DIM,SAMPLESTYPE>  (*pInterpolateBins)(const Vector<DIM,SAMPLESTYPE> &, const Vector<DIM,SAMPLESTYPE> &, double alpha), 
				 double pkNNDist=2.0, unsigned int pNLevels=1) :
		...
		numberOfNN(my_min((unsigned int)pwA.size(),my_min((unsigned int)pwB.size(),my_max((unsigned int)500,(unsigned int)floor(pkNNDist+2.5)))))		// number of nearest neighbors to precompute
	{ };
...
#else
// --added for ubuntu multithreading-- //
			mProcessNum = sysconf( _SC_NPROCESSORS_ONLN );

			std::cout << "number of cores: " << mProcessNum << std::endl;

			pthread_t threads[2];

			pthread_create(&threads[0], NULL, computeNNdist<DIM,SAMPLESTYPE>, (void*) &dataA);
			pthread_create(&threads[1], NULL, computeNNdist<DIM,SAMPLESTYPE>, (void*) &dataB);
...
// --added for ubuntu multithreading-- //
			...
			pthread_create(&threads[0], NULL, computeKernelMatrix<DIM,SAMPLESTYPE>, (void*) &dataKerA);
			pthread_create(&threads[1], NULL, computeKernelMatrix<DIM,SAMPLESTYPE>, (void*) &dataKerB);
...
//			computeKernelMatrix<DIM,SAMPLESTYPE>((void*) &dataKerB);
			...
			std::vector<double> signalA, signalB;
			...
			{
				// filter data
				std::vector<double> filteredwA(wA.size(),0.), filteredwB(wB.size(),0.);
...
// --added for ubuntu multithreading-- //
						...
						pthread_create(&threads[0], NULL, filterSamples<DIM,SAMPLESTYPE>, (void*) &filtDataA);
						pthread_create(&threads[1], NULL, filterSamples<DIM,SAMPLESTYPE>, (void*) &filtDataB);
...
// --added for ubuntu multithreading-- //
					...
					pthread_create(&threads[0], NULL, computeRBFs<DIM,SAMPLESTYPE>, (void*) &rbfA);
					pthread_create(&threads[1], NULL, computeRBFs<DIM,SAMPLESTYPE>, (void*) &rbfB);
...
#else
			unsigned int nLaunch = min(mProcessNum, resultFlow.size());
			pthread_t* threads = new pthread_t[nLaunch];
#endif
...
// --added for ubuntu multithreading-- //
				...
				for (unsigned int k=0; k<numLaunchReal; k++)
					pthread_create(&threads[k], NULL, interpMode<DIM,SAMPLESTYPE>, (void*) &dataProc[k]);  
...
// --modified for ubuntu multithreading-- //
//#ifdef WIN32
			delete[] threads;
//#endif
		}
...
private:
	...
	std::vector<Vector<DIM,SAMPLESTYPE> > A;
...
struct	NNDistData
{
	NNDistData(std::vector<Vector<DIM,SAMPLESTYPE> > &pSamples, unsigned int pNnearD, std::vector<std::vector<double> > &pDists, std::vector<std::vector<int> > &pNnid, double (*pSqrDist)(const Vector<DIM,SAMPLESTYPE> &, const Vector<DIM,SAMPLESTYPE> &)):
	samples(pSamples),NnearD(pNnearD), dists(pDists), nnid(pNnid), sqrDist(pSqrDist)
	{		
	}
